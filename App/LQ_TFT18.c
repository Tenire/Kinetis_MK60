...
#define TFT_SCL(x) gpio_set(TFT_SCL_PIN,x)
...
{ 	
	...
	TFTSPI_Write_Cmd(0xB4); //Dot inversion
	...
	TFTSPI_Write_Byte(0x0D);
	TFTSPI_Write_Byte(0x00);
	TFTSPI_Write_Cmd(0xC3);
	TFTSPI_Write_Byte(0x8D);
	...
	TFTSPI_Write_Cmd(0x36); //MX, MY, RGB mode
	if(type) 
...
void TFTSPI_Write_Cmd(uint8_t cmd)
{
	...
	TFT_DC(0);// A0=0 ILI9163_A0=0;	 //A0=0发送命令  
	for(i=0;i<8;i++)
	{
		TFT_SCL(0);//tft18delay_1us(1);	// SCK=0  ILI9163_SCK=0;
		if(cmd&0x80){TFT_SDA(1);}// SDI=1
		else{TFT_SDA(0);}// SDI=0 
		TFT_SCL(1);//tft18delay_1us(1);	// SCK=1  ILI9163_SCK = 1;
		cmd = (cmd<<1);
	}
}
...
{  
	...
	TFT_DC(1);// A0=1  ILI9163_A0=1;		  //A0=1发送数据  				
	for(i=0;i<8;i++)
	{
		TFT_SCL(0);tft18delay_1us(1);	// SCK=0  ILI9163_SCK=0; 	    
		if(dat&0x80){TFT_SDA(1);}// SDI=1
		else{TFT_SDA(0);}// SDI=0             
		TFT_SCL(1);;tft18delay_1us(1);	// SCK=1 ILI9163_SCK = 1;
		dat = (dat<<1);    
	}
...
{  		 
	...
	TFT_DC(1);// A0=1  ILI9163_A0=1;	
	
	for(i=0;i<16;i++)
	{
		TFT_SCL(0);//tft18delay_1us(1);	// SCK=0  ILI9163_SCK=0;  
		if(dat&0x8000){ TFT_SDA(1);}// SDI=1
		else{TFT_SDA(0);}// SDI=0
		TFT_SCL(1);//tft18delay_1us(1);	// SCK=1  ILI9163_SCK=1;
		dat<<=1;    
	}  
...
{     
	"科创未来智能车实验室 北京龙邱丘智能科技淘宝店液晶单片机智能车器人启动中电压量不足请稍候，！：℃攻击娱乐模式普通格斗对话俯卧撑倒立握手敬礼跳舞蹈运动蓝精灵太极街舞温度—"
};
...
const unsigned char hanzi16x16[] = {	
	...
	0x34,0x10,0x50,0x1E,0x53,0xF0,0x90,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,//科0
	
	0x08,0x04,0x08,0x04,0x14,0x04,0x12,0x24,0x21,0x24,0x40,0xA4,0xBE,0x24,0x22,0x24,
	0x22,0x24,0x22,0x24,0x2A,0x24,0x24,0x24,0x20,0x84,0x20,0x84,0x1F,0x94,0x00,0x08,//创1
	
	0x01,0x00,0x01,0x00,0x01,0x00,0x3F,0xF8,0x01,0x00,0x01,0x00,0x01,0x00,0xFF,0xFE,
	0x03,0x80,0x05,0x40,0x09,0x20,0x11,0x10,0x21,0x08,0xC1,0x06,0x01,0x00,0x01,0x00,//未2
	
	0x01,0x00,0x01,0x00,0x01,0x00,0x7F,0xFC,0x01,0x00,0x11,0x10,0x09,0x10,0x09,0x20,
	0xFF,0xFE,0x03,0x80,0x05,0x40,0x09,0x20,0x31,0x18,0xC1,0x06,0x01,0x00,0x01,0x00,//来3
	
	0x20,0x00,0x3E,0x7C,0x48,0x44,0x08,0x44,0xFF,0x44,0x14,0x44,0x22,0x7C,0x40,0x00,
	0x1F,0xF0,0x10,0x10,0x10,0x10,0x1F,0xF0,0x10,0x10,0x10,0x10,0x1F,0xF0,0x10,0x10,//智4
	
	0x10,0x40,0x24,0x44,0x42,0x48,0xFF,0x70,0x01,0x40,0x00,0x42,0x7E,0x42,0x42,0x3E,
	0x42,0x00,0x7E,0x44,0x42,0x48,0x42,0x70,0x7E,0x40,0x42,0x42,0x4A,0x42,0x44,0x3E,//能5
	
	0x02,0x00,0x02,0x00,0x02,0x00,0x7F,0xFC,0x04,0x00,0x09,0x00,0x11,0x00,0x21,0x00,
	0x3F,0xF8,0x01,0x00,0x01,0x00,0xFF,0xFE,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,//车6
	
	0x02,0x00,0x01,0x00,0x7F,0xFE,0x40,0x02,0x88,0x84,0x04,0x80,0x04,0x80,0x10,0x80,
	0x08,0x80,0x08,0x80,0xFF,0xFE,0x01,0x40,0x02,0x20,0x04,0x10,0x18,0x08,0x60,0x04,//实7
	
	0x00,0x20,0xF8,0x20,0x08,0x50,0x48,0x50,0x48,0x88,0x49,0x04,0x4A,0xFA,0x7C,0x00,
	0x04,0x44,0x04,0x24,0x1D,0x24,0xE4,0xA8,0x44,0x88,0x04,0x10,0x2B,0xFE,0x10,0x00,//验8
	
	0x02,0x00,0x01,0x00,0x7F,0xFE,0x40,0x02,0x80,0x04,0x3F,0xF8,0x04,0x00,0x08,0x20,
	0x1F,0xF0,0x01,0x10,0x01,0x00,0x3F,0xF8,0x01,0x00,0x01,0x00,0xFF,0xFE,0x00,0x00,//室9
...
{
	//TFTSPI_Show_Pic2(xs,ys,160,90,code);       //80,gImage_ftflogo);     //
	draw_picture(xs,ys,160,90,code);
}
...
{ 
	...
	TFTSPI_CLS(u16CYAN);   //蓝色屏幕	
	TFTSPI_Show_Logo(0,37);//显示龙邱LOG  
	TFTSPI_P16x16Str(0,0,"科创未来智能车实验室",u16RED,u16CYAN);		//字符串显示
	TFTSPI_P8X16Str(0,1,"www.createfuture.top",u16WHITE,u16BLACK);		//字符串显示  
	char txt[32];
	int count = 1;
	...
	while(1)
	{
		sprintf(txt, "T:%4.1d", count);                    //将变量填充到字符串的对应位置，并将字符串存放到txt[]中
		TFTSPI_P8X16Str(0, 2, txt, u16RED, u16CYAN);              //将txt中 内容显示出来
		delayms(1000);
		count++;      
	}
}//TEST()