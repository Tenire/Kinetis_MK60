...
int mpu_gyro_x=0,mpu_gyro_y=0,mpu_gyro_z=0;
int mpu_acc_x=0,mpu_acc_y=0,mpu_acc_z=0;
...
{
      simiic_write_reg(MPU6050_DEV_ADDR, PWR_MGMT_1, 0x00);	   //解除休眠状态       // 原来0x07
      simiic_write_reg(MPU6050_DEV_ADDR, SMPLRT_DIV, 0x03);      //1000HZ采样率    当SMPLRT_DIV为0x07采样率为125hz，目前为0x19
      simiic_write_reg(MPU6050_DEV_ADDR, CONFIG, 0x04);          //
      simiic_write_reg(MPU6050_DEV_ADDR, GYRO_CONFIG, 0x18);     //2000
      simiic_write_reg(MPU6050_DEV_ADDR, ACCEL_CONFIG, 0x00);    //8g
...
{
    uint8 L;   uint16 H ;
    H=simiic_read_reg(MPU6050_DEV_ADDR, REG_Address,I2C0);
    L=simiic_read_reg(MPU6050_DEV_ADDR, REG_Address+1,I2C0);
    return (H<<8)+L;   //合成数据
}
